...
import fii.jt.lab3.model.Match;
    ...
    public MatchMaker(List<Team> teams) {
        ...
        for (int i = 0; i < teams.size() - 1; i++) {
            for (int j = 0; j < teams.size() / 2; j++) {
                pairings.add(new Tournament(i, teams.get(tournament[i][j].getFirstTeam().intValue()).getName(), teams.get(tournament[i][j].getSecondTeam().intValue()).getName()));
            }
        }
    }
